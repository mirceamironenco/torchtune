...
def qwen3_moe(
    ...
    - Token embeddings
    - num_layers number of TransformerSelfAttentionLayer blocks with MoE layers
    - RMS Norm layer applied to the output of the transformer
    - Final projection into token space
    ...
    Args:
        ...
        num_experts_per_token (int): number of experts each token will be routed to.
        max_seq_len (int): maximum sequence length the model will be run with, as used
            by :func:`~torchtune.modules.KVCache`
        head_dim (Optional[int]): Dimension of each attention head. If not
            specified, it defaults to `embed_dim // num_heads`. In GQA, `head_dim` is not necessarily equal to
...
def qwen3_moe_mlp(
    dim: int, hidden_dim: int, num_experts: int, experts_per_token: int
) -> MoE:
    """
    Build the MoE layer associated with the Qwen3 MoE model.

    Args:
        dim (int): input dimension.
        hidden_dim (int): intermediate dimension for each expert.
        num_experts (int): number of experts in the MoE layer.
        experts_per_token (int): number of experts each token will be routed to.

    Returns:
        MoE: Instantiation of Qwen3 MoE layer.
    """
    router = TokenChoiceTopKRouter(
        ...
        normalize_topk_probs=True,
        score_function="softmax",
    )
    experts = GroupedExperts(dim=dim, hidden_dim=hidden_dim, num_experts=num_experts)
    return MoE(experts=experts, router=router, router_score_scaling="output")
